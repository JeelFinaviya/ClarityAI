...
export function ResultsView({
  ...
  const getRankBadge = (s) => {
    ...
    if (s >= 71) return { title: 'Mechanics Specialist', icon: '⚔️', color: 'text-cyan-400', border: 'border-cyan-500/40', bg: 'bg-cyan-500/10' };
    if (s >= 46) return { title: 'Concept Explorer', icon: '🛡️', color: 'text-indigo-400', border: 'border-indigo-500/40', bg: 'bg-indigo-500/10' };
    ...
    <div className="w-full flex-1 flex flex-col items-center justify-start arena-bg-radial px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      <div className="w-full max-w-5xl flex flex-col gap-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-white/10">
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={onRefine}
              className="px-4 py-2 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 text-slate-300 text-xs font-semibold transition-all cursor-pointer"
            >
              Modify Explanation
            </button>
            ...
            <button
              type="button"
              onClick={onReset}
              className="px-5 py-2 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 text-white text-xs font-bold shadow-md shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer"
            >
        ...
        <div className="glass-card glass-card-glow rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="relative w-36 h-36 flex items-center justify-center shrink-0">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 140 140">
                ...
                <circle
                  ...
                  stroke={score >= 80 ? '#10b981' : score >= 60 ? '#06b6d4' : score >= 45 ? '#f59e0b' : '#f43f5e'}
                  strokeWidth="10"
          ...
          <div className="w-full md:w-72 bg-slate-900/90 rounded-2xl p-4 border border-white/10 flex flex-col gap-3 shrink-0">
            <div className="flex items-center justify-between text-xs font-bold text-slate-300">
              ...
              <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-mono-code ${
                calStatus === 'calibrated' ? 'bg-emerald-500/20 text-emerald-300' :
                calStatus === 'overconfident' ? 'bg-amber-500/20 text-amber-300' : 'bg-blue-500/20 text-blue-300'
              }`}>
                {calStatus}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-center py-1">
              <div className="bg-slate-800/60 rounded-xl p-2">
                ...
                <div className="font-display text-lg font-bold text-indigo-400 font-mono-code">{statedConf}%</div>
              </div>
              <div className="bg-slate-800/60 rounded-xl p-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="glass-card rounded-2xl p-5 border border-white/10 flex flex-col justify-between">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="font-display text-lg font-bold text-cyan-400 font-mono-code">
                {dims.core_accuracy ?? dims.depth_of_explanation?.score ?? score}%
              </span>
            </div>
            <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden mb-3">
              <div className="h-full bg-cyan-400 rounded-full" style={{ width: `${dims.core_accuracy ?? score}%` }} />
            </div>
          ...
          <div className="glass-card rounded-2xl p-5 border border-white/10 flex flex-col justify-between">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="font-display text-lg font-bold text-indigo-400 font-mono-code">
                {dims.causal_depth ?? dims.causal_relationships?.score ?? score}%
              </span>
            </div>
            <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden mb-3">
              <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${dims.causal_depth ?? score}%` }} />
            </div>
            <p className="text-xs text-slate-400 leading-relaxed">
        ...
        {journey && (
          <div className="glass-card rounded-2xl p-6 sm:p-7 border border-indigo-500/20 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
              ...
              <div className="p-3.5 rounded-xl bg-slate-900/70 border border-white/5">
                <div className="font-bold text-cyan-400 uppercase tracking-wider text-[10px] mb-1">3. Follow-up Finding</div>
            ...
            {conceptsMapped.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {conceptsMapped.map((c, i) => (
                  <div key={i} className="p-3.5 rounded-xl bg-slate-900/80 border border-white/5 flex flex-col gap-1.5">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-emerald-300 font-display">{c.concept}</span>
            ...
            {conceptsUnexplored.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {conceptsUnexplored.map((c, i) => (
                  <div key={i} className="p-3.5 rounded-xl bg-slate-900/80 border border-white/5 flex flex-col gap-1">
          ...
          <div className="glass-card rounded-2xl p-6 border border-rose-500/30 flex flex-col gap-4 bg-rose-950/10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {misconceptions.map((m, i) => (
                <div key={i} className="p-4 rounded-xl bg-slate-900/90 border border-rose-500/20 flex flex-col gap-2">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-white/10">
          ...
          <button
            type="button"
            onClick={onReset}
            className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-500 text-white font-display text-sm font-bold shadow-xl shadow-indigo-500/30 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
